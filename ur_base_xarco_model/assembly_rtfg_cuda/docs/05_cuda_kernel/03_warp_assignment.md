...
│   │                           Lane 21-35: 额外计算 15 个元素（当前使用 36 线程计算全部 36 个元素）
│   │                           Lane 36-63: 闲置（Warp 2 仅 64 线程中的前 36 个活跃）
│   │   效率: ~56%（36/64 线程活跃）
│   │
...
- `H[row][col] = H[col][row]`
- 上三角元素数: `6×7/2 = 21` 个
- 当前使用 36 线程计算所有 36 个元素（上三角 + 下三角对称填充）
- 36 个线程跨 Warp 2 和 Warp 3（线程 64-99），仍在 2 个 warp 内
...
| 2 (Hessian) | 36 (跨 2 warp) | 32 | 112% | 当前使用 36 线程计算全部 36 个元素 |
