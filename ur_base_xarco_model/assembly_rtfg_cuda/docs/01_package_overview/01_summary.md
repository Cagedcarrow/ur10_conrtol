...
1. **GPU 批处理 IK 求解器**: 将 52,416 个独立 IK 任务映射到 GPU 线程层次结构，实现 960× 加速比
...
| 指标 | 值 |
|------|-----|
| IK 批处理 (273 目标) | 6.434 ms (GPU Kernel) |
| 单目标平均 | 24 μs |
| 平均迭代次数 | 6.7 |
| 加速比 (vs CPU C++) | **960×** (6.2 s → 6.434 ms) |
| 寄存器/线程 | 96 (零溢出) |
| 共享内存/Block | 1,616 bytes |
| Bank 冲突 | 0 |
| FP64 算术强度 | 150 FLOP/Byte (168× Ridge Point) |
